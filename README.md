...
PCB for the hardware layer of the vinyl record shelf finder: a system that illuminates the correct cube of a Kallax 4×4 shelf when you look up a vinyl record. The shelf-finder application lives in [vinyl-record-shelf-gopher](https://github.com/SimplicityGuy/vinyl-record-shelf-gopher) and uses record metadata from [discogsography](https://github.com/SimplicityGuy/discogsography).
...
Power:
- 5V in via J1 (CUI PJ-102AH barrel jack, center positive)
...
Each cube maps to one WLED segment. The [vinyl-record-shelf-gopher](https://github.com/SimplicityGuy/vinyl-record-shelf-gopher) service sends an HTTP request to activate the segment corresponding to the record's cube location. API specification: TBD.
...
⚠️ **Center positive only.** Verify polarity with a multimeter before first power-up.
...
WLED firmware is flashed to the XIAO ESP32C6 before installation. The shelf-finder application lives in [vinyl-record-shelf-gopher](https://github.com/SimplicityGuy/vinyl-record-shelf-gopher), which consumes record metadata from [discogsography](https://github.com/SimplicityGuy/discogsography) and issues the WLED segment calls to this controller. API specification: TBD.
